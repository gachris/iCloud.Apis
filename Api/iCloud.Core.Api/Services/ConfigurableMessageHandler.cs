...
namespace iCloud.Apis.Core.Services
{
    public class ConfigurableMessageHandler : DelegatingHandler
    {
        ...
        /// <summary>The User-Agent suffix header which contains the <see cref="F:ICloud.Apis.Http.ConfigurableMessageHandler.ApiVersion" />.</summary>
        private static readonly string UserAgentSuffix = "ICloud-api-dotnet-client/" + ConfigurableMessageHandler.ApiVersion + " (gzip)";
        ...
        /// <summary>A list of <see cref="T:ICloud.Apis.Http.IHttpUnsuccessfulResponseHandler" />.</summary>
        ...
        /// <summary>A list of <see cref="T:ICloud.Apis.Http.IHttpExceptionHandler" />.</summary>
        private readonly IList<IHttpExceptionHandler> exceptionHandlers = new List<IHttpExceptionHandler>();
        /// <summary>A list of <see cref="T:ICloud.Apis.Http.IHttpExecuteInterceptor" />.</summary>
        ...
        /// <summary>
        /// Gets a list of <see cref="T:ICloud.Apis.Http.IHttpUnsuccessfulResponseHandler" />s.
        /// <remarks>
        /// Since version 1.10, <see cref="M:ICloud.Apis.Http.ConfigurableMessageHandler.AddUnsuccessfulResponseHandler(ICloud.Apis.Http.IHttpUnsuccessfulResponseHandler)" /> and
        /// <see cref="M:ICloud.Apis.Http.ConfigurableMessageHandler.RemoveUnsuccessfulResponseHandler(ICloud.Apis.Http.IHttpUnsuccessfulResponseHandler)" /> were added in order to keep this class thread-safe.
        ...
        /// <summary>
        /// Gets a list of <see cref="T:ICloud.Apis.Http.IHttpExceptionHandler" />s.
        /// <remarks>
        /// Since version 1.10, <see cref="M:ICloud.Apis.Http.ConfigurableMessageHandler.AddExceptionHandler(ICloud.Apis.Http.IHttpExceptionHandler)" /> and <see cref="M:ICloud.Apis.Http.ConfigurableMessageHandler.RemoveExceptionHandler(ICloud.Apis.Http.IHttpExceptionHandler)" /> were added
        ...
        /// <summary>
        /// Gets a list of <see cref="T:ICloud.Apis.Http.IHttpExecuteInterceptor" />s.
        /// <remarks>
        /// Since version 1.10, <see cref="M:ICloud.Apis.Http.ConfigurableMessageHandler.AddExecuteInterceptor(ICloud.Apis.Http.IHttpExecuteInterceptor)" /> and <see cref="M:ICloud.Apis.Http.ConfigurableMessageHandler.RemoveExecuteInterceptor(ICloud.Apis.Http.IHttpExecuteInterceptor)" /> were
        ...
        /// <see cref="T:ICloud.Apis.Http.IHttpUnsuccessfulResponseHandler" /> or <see cref="T:ICloud.Apis.Http.IHttpExceptionHandler" /> which handles the
        ...
        /// <remarks>
        /// The number of allowed redirects (3xx) is defined by <see cref="P:ICloud.Apis.Http.ConfigurableMessageHandler.NumRedirects" />. This property defines
        /// only the allowed tries for &gt;=400 responses, or when an exception is thrown. For example if you set
        /// <see cref="P:ICloud.Apis.Http.ConfigurableMessageHandler.NumTries" /> to 1 and <see cref="P:ICloud.Apis.Http.ConfigurableMessageHandler.NumRedirects" /> to 5, the library will send up to five redirect
        ...
        /// See <see cref="P:ICloud.Apis.Http.ConfigurableMessageHandler.NumTries" /> for more information.
        /// </summary>
        ...
        /// with <see cref="P:ICloud.Apis.Http.ConfigurableMessageHandler.ApplicationName" /> and the library version. It also calls interceptors before each attempt,
        ...
        {
            bool loggable = this.IsLoggingEnabled && ConfigurableMessageHandler.Logger.IsDebugEnabled;
            ...
            do
            {
                ...
                if (response == null)
                {
                    ...
                    lock (this.exceptionHandlersLock)
                        list2 = this.exceptionHandlers.ToList<IHttpExceptionHandler>();
                    ...
                    if (!flag1)
                    {
                        ConfigurableMessageHandler.Logger.Error(lastException, "Exception was thrown while executing a HTTP request and it wasn't handled");
                        throw lastException;
                    }
                    if (loggable)
                        ConfigurableMessageHandler.Logger.Debug("Exception {0} was thrown, but it was handled by an exception handler", (object)lastException.Message);
                }
                    ...
                    if (!flag1)
                    {
                        if (this.FollowRedirect && this.HandleRedirect(response))
                        {
                            if (redirectRemaining-- == 0)
                                triesRemaining = 0;
                            if (loggable)
                                ConfigurableMessageHandler.Logger.Debug("Redirect response was handled successfully. Redirect to {0}", (object)response.Headers.Location);
                        }
                        else
                        {
                            if (loggable)
                                ConfigurableMessageHandler.Logger.Debug("An abnormal response wasn't handled. Status code is {0}", (object)response.StatusCode);
                            triesRemaining = 0;
                        }
                    }
                    else if (loggable)
                        ConfigurableMessageHandler.Logger.Debug("An abnormal response was handled by an unsuccessful response handler. Status Code is {0}", (object)response.StatusCode);
                }
            }
            while (triesRemaining > 0);
            if (response == null)
            {
                ConfigurableMessageHandler.Logger.Error(lastException, "Exception was thrown while executing a HTTP request");
                throw lastException;
            }
            if (!response.IsSuccessStatusCode)
                ConfigurableMessageHandler.Logger.Debug("Abnormal response is being returned. Status Code is {0}", (object)response.StatusCode);
            return response;
        }
