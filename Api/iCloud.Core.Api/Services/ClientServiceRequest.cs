...
namespace iCloud.Apis.Core.Services
{
    ...
    public abstract class ClientServiceRequest<TResponse> : IClientServiceRequest<TResponse>, IClientServiceRequest
    {
        ...
        /// <see cref="P:ICloud.Apis.Requests.ClientServiceRequest`1.RequestParameters" /> dictionary.
        /// </summary>
        ...
        protected virtual async Task<TResponse> ParseResponse(HttpResponseMessage response)
        {
            ...
            RequestError requestError = await this._service.DeserializeError(response).ConfigureAwait(false);
            throw new ICloudApiException(this._service.Name, requestError.ToString())
            {
                Error = requestError,
        ...
        public HttpRequestMessage CreateRequest(bool? overrideGZipEnabled = null)
        {
            ...
            this.SetHttpRequestMessageHeader(request);
            object body = this.GetBody();
            request.SetRequestSerailizedContent(this._service, body, overrideGZipEnabled.HasValue ? overrideGZipEnabled.Value : this._service.GZipEnabled, ContentType);
            this.AddETag(request);
            return request;
        }

        private void SetHttpRequestMessageHeader(HttpRequestMessage request)
        {
        ...
        /// <summary>
        /// Creates the <see cref="T:ICloud.Apis.Requests.RequestBuilder" /> which is used to generate a request.
        /// </summary>
        /// <returns>
        ...
        private RequestBuilder CreateBuilder()
        {
            ...
            requestBuilder.AddParameter(RequestParameterType.Query, "key", this._service.ApiKey);
        ...
        private void AddETag(HttpRequestMessage request)
        {
            ...
            {
                ClientServiceRequest<TResponse>.Logger.Error(ex, "Can't set {0}. Etag is: {1}.", etagAction, etag);
            }
        }
